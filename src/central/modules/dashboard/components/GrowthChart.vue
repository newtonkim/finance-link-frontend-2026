...
// Brand navy used across the module's primary controls.
const line = computed(() => (isDark.value ? '#53B3DA' : '#052659'))
  ...
  datasets: [
    {
      ...
      pointBorderColor: isDark.value ? '#171717' : '#ffffff',
      pointBorderWidth: 2,
      fill: true,
      backgroundColor: (ctx: any) => {
        ...
        g.addColorStop(0, isDark.value ? 'rgba(83,179,218,0.22)' : 'rgba(5,38,89,0.14)')
...
const chartOptions = computed<any>(() => ({
  ...
  plugins: {
    ...
    tooltip: {
      backgroundColor: isDark.value ? '#262626' : '#0a0a0a',
      padding: 10,
      cornerRadius: 8,
      displayColors: false,
...
<template>
  ...
  >
    <div class="flex items-center justify-between gap-3 px-5 py-4 border-b border-neutral-200 dark:border-neutral-800">
      <h2 id="growth-heading" class="text-base font-black tracking-tight">Tenant Growth</h2>
      <span class="text-[11px] font-semibold uppercase tracking-wider text-neutral-400">Last 12 months</span>
    </div>

    <div class="min-h-[260px] flex-1 px-3 pb-5">
      <div v-if="loading" class="mx-2 h-[240px] animate-pulse rounded-xl bg-neutral-100 dark:bg-neutral-800"></div>

      <div
        v-else-if="!hasData"
        class="flex h-[240px] flex-col items-center justify-center gap-1 px-6 text-center"
      >
        <p class="text-sm font-bold">No tenants yet</p>
