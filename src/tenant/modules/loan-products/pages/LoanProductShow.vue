...
function processingFeeLabel() {
  ...
  return moneyOrDash(
    product.value.processing_fee_value_formatted,
    product.value.processing_fee_value,
  )
}
</script>
  ...
  <div class="flex h-full flex-1 flex-col gap-6 p-4 md:p-6 bg-[#f8faf9] dark:bg-[#0a0a0a]">
    ...
    <template v-else-if="product">
      ...
      >
        ...
        <div class="grid gap-4 sm:grid-cols-2">
          <div class="sm:col-span-2 flex items-center gap-3">
            ...
            <span
              v-if="product.is_in_use"
              class="inline-flex rounded-full bg-amber-50 px-2.5 py-0.5 text-xs font-medium text-amber-700 dark:bg-amber-900/30 dark:text-amber-300"
            >
          ...
          <div>
            ...
            <p class="text-sm text-neutral-900 dark:text-white">
              {{ moneyOrDash(product.min_amount_formatted, product.min_amount) }}
            </p>
          </div>
          <div>
            <p class="text-xs font-medium text-neutral-500 dark:text-neutral-400 mb-1">
            ...
            <p class="text-sm text-neutral-900 dark:text-white">
              {{ moneyOrDash(product.max_amount_formatted, product.max_amount) }}
            </p>
          </div>
        </div>
      </div>
      ...
      >
        ...
        <div class="grid gap-4 sm:grid-cols-3">
          <div>
            ...
            <p class="text-sm text-neutral-900 dark:text-white">
              {{ interestMethodLabel(product.interest_method) }}
            </p>
          </div>
          <div>
            <p class="text-xs font-medium text-neutral-500 dark:text-neutral-400 mb-1">
            ...
            <p class="text-sm text-neutral-900 dark:text-white">
              {{ orDash(product.repayment_structure) }}
            </p>
          </div>
          <div>
            <p class="text-xs font-medium text-neutral-500 dark:text-neutral-400 mb-1">
          ...
          <div>
            ...
            <p class="text-sm text-neutral-900 dark:text-white capitalize">
              {{ orDash(product.interest_period) }}
            </p>
          </div>
        </div>
      </div>
      ...
      >
        ...
        <div class="grid gap-4 sm:grid-cols-3">
          <div>
            ...
            <p class="text-sm text-neutral-900 dark:text-white">
              {{
                product.loan_duration != null
                  ? `${product.loan_duration} ${product.duration_type ?? ''}`
                  : '—'
              }}
            </p>
          </div>
          <div>
            ...
            <p class="text-sm text-neutral-900 dark:text-white capitalize">
              {{ orDash(product.repayment_cycle) }}
            </p>
          </div>
          <div>
            <p class="text-xs font-medium text-neutral-500 dark:text-neutral-400 mb-1">
      ...
      >
        ...
        <div
          v-if="!product.penalty_rules?.length"
          class="rounded-xl border border-dashed border-neutral-200 dark:border-neutral-700 py-8 text-center text-sm text-neutral-400 dark:text-neutral-500"
        >
        ...
        <div
          v-else
          class="overflow-hidden rounded-xl border border-neutral-100 dark:border-neutral-800"
        >
          <table class="w-full text-sm">
            <thead
              class="border-b border-neutral-100 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-800/50"
            >
              <tr
            ...
            <tbody class="divide-y divide-neutral-50 dark:divide-neutral-800">
              <tr v-for="(rule, idx) in product.penalty_rules" :key="idx">
                <td class="px-4 py-3 text-neutral-900 dark:text-white">
                  {{ orDash(rule.penalty_type) }}
                </td>
                <td class="px-4 py-3 text-neutral-600 dark:text-neutral-400">
                  {{ orDash(rule.penalty_rate) }}
                </td>
                <td class="px-4 py-3 text-neutral-600 dark:text-neutral-400">
                  {{ orDash(rule.grace_days) }}
                </td>
                <td class="px-4 py-3 text-neutral-600 dark:text-neutral-400">
                  {{ moneyOrDash(rule.amount_formatted, rule.amount) }}
                </td>
              </tr>
            </tbody>
          </table>
