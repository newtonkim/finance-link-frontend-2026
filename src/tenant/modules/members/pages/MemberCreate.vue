...
import { ArrowLeft, UserCircle2, Wallet } from 'lucide-vue-next'
import { Label, InputError, Spinner } from '@/Global'
import { formatMoneyValue } from '@/Global'
import PhoneInput from '@/Global/PhoneInput.vue'
import SearchableSelect from '@/Global/SearchableSelect.vue'
import MemberSharePurchase from '../components/MemberSharePurchase.vue'
...
const nationalityOptions = [
  { id: 'Ugandan', name: 'Uganda' },
  { id: 'Kenyan', name: 'Kenya' },
  { id: 'Tanzanian', name: 'Tanzania' },
  { id: 'Rwandan', name: 'Rwanda' },
  { id: 'Burundian', name: 'Burundi' },
  { id: 'South Sudanese', name: 'South Sudan' },
  { id: 'Congolese', name: 'DR Congo' },
  { id: 'Ethiopian', name: 'Ethiopia' },
  { id: 'Somali', name: 'Somalia' },
  { id: 'Nigerian', name: 'Nigeria' },
  { id: 'Ghanaian', name: 'Ghana' },
  { id: 'South African', name: 'South Africa' },
  { id: 'British', name: 'United Kingdom' },
  { id: 'American', name: 'United States' },
  { id: 'Indian', name: 'India' },
  { id: 'Other', name: 'Other' },
]
...
// ─── Shared input class ───────────────────────────────────────────────────────
const inputCls =
  'w-full rounded-xl border border-neutral-200 bg-white px-4 py-3 text-sm text-neutral-800 outline-none placeholder:text-neutral-400 focus:border-neutral-400 focus:ring-1 focus:ring-neutral-300'
</script>
      ...
      <form @submit.prevent="submit">
        ...
        <div class="grid grid-cols-1 gap-x-8 gap-y-6 sm:grid-cols-2">
          ...
          <template v-else>
            ...
            <div class="grid gap-1.5">
              <Label
                >Profile Picture <span class="text-neutral-400 font-normal">(Optional)</span></Label
              >
          ...
          <div
            v-if="settingsStore.autoCreateSavingsAccount && !settingsStore.hideInitialDeposit"
            class="grid gap-1.5"
          >
            <Label for="initial_deposit">Initial deposit <span class="text-red-500">*</span></Label>
            <div
              class="flex overflow-hidden rounded-xl border border-neutral-200 bg-white focus-within:border-neutral-400 focus-within:ring-1 focus-within:ring-neutral-300"
            >
              <span
          ...
          <div class="grid gap-1.5">
            <Label for="joined_at"
              >Date Joined <span class="text-neutral-400 font-normal">(Optional)</span></Label
            >
          ...
          <div class="sm:col-span-2 grid gap-1.5">
            <Label
              ...
              <span class="text-neutral-400 font-normal"
                >(Staff who brought this member)</span
              ></Label
            >
            <SearchableSelect
              v-model="form.referred_by"
              :options="
                staffList.map((s) => ({ id: s.id, name: s.name + (s.role ? ' · ' + s.role : '') }))
              "
          ...
          <template v-if="isExisting">
            ...
            <div
              v-if="settingsStore.autoCreateSavingsAccount && !settingsStore.hideOpeningBalance"
              class="grid gap-1.5"
            >
              <Label for="opening_balance"
                >Opening Balance <span class="text-red-500">*</span></Label
              >
              <div
                class="flex overflow-hidden rounded-xl border border-neutral-200 bg-white focus-within:border-neutral-400 focus-within:ring-1 focus-within:ring-neutral-300"
              >
        ...
        <div
          v-if="!settingsStore.autoCreateSavingsAccount"
        ...
        <MemberSharePurchase
          v-if="sharesRequired"
          v-model:shares-quantity="form.shares_quantity"
          :shares-error="sharesError"
          :api-error="errors.shares_quantity ?? ''"
        />
