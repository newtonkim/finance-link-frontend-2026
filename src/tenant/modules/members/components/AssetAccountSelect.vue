...
async function fetchAccounts() {
  ...
  try {
    ...
    accounts.value = data
      .filter((opt: any) => {
        const name = opt.name || '';
        const isMatch = allowedPatterns.some(pattern => pattern.test(name));
        const isAutoSelected = props.autoSelectId === opt.id;
        const isModelValue = props.modelValue === opt.id;
        return isMatch || isAutoSelected || isModelValue;
      })
      .slice()
      .sort((a: Account, b: Account) => a.gl_code.localeCompare(b.gl_code))
  } finally {
    loading.value = false
  }
}
...
const createFooterLabel = computed(() => {
  const q = search.value.trim()
  return q ? `Create "${q}" as asset account` : 'Create new asset account'
})
  ...
  <div class="relative w-full">
    ...
    >
      <span :class="selectedLabel ? '' : 'text-neutral-400'">
        {{ selectedLabel || placeholder || 'Select asset account' }}
      </span>
