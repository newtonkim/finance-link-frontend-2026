  ...
  <div class="flex flex-col gap-8 p-10 min-h-screen bg-[#f8fafc] dark:bg-neutral-950 font-sans">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6">
      <div class="space-y-2">
        ...
        <nav class="flex items-center gap-2 text-[11px] font-bold uppercase tracking-widest text-neutral-400">
          <RouterLink to="/tenant/dashboard" class="hover:text-[#0050D8] transition-colors">Dashboard</RouterLink>
          <ChevronRight class="h-3 w-3" />
          <span class="text-[#0050D8]">Group Savings -</span>
        </nav>
        <h1 class="text-4xl font-black text-[#0050D8] dark:text-white tracking-tight">Group Savings</h1>
      ...
      <button
        class="bg-[#052659] hover:bg-[#052659]/90 text-white border-0 inline-flex items-center gap-2.5 rounded-2xl  px-8 py-4 text-sm font-bold text-white hover:bg-[#052659]/90 transition-all shadow-sm shadow-sm active:scale-95 group"
        @click="openCreate"
      >
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
      <div v-for="(val, label) in { 'Total Groups': groups.length, 'Active Portfolios': 0, 'Growth Rate': '0%' }" :key="label" class="bg-white dark:bg-neutral-900 p-6 rounded-[24px] border border-neutral-100 dark:border-neutral-800 shadow-sm">
        ...
        <p class="text-2xl font-black text-[#0050D8] dark:text-white">{{ val }}</p>
      </div>
    </div>
    ...
    <div class="flex flex-col sm:flex-row items-center gap-4 bg-white dark:bg-neutral-900 p-4 rounded-[28px] border border-neutral-100 dark:border-neutral-800 shadow-sm">
      <div class="relative flex-1">
        ...
        <input
          ...
          class="w-full rounded-2xl border-none bg-neutral-50 dark:bg-neutral-800 py-3.5 pl-12 pr-4 text-sm outline-none transition-all placeholder:text-neutral-400 focus:bg-neutral-100 focus:ring-2 focus:ring-[#0050D8]/5 dark:text-white"
        />
      </div>
        ...
        <table class="w-full text-sm border-collapse">
          <thead>
            <tr class="bg-neutral-50/50 dark:bg-neutral-800/50">
              <th class="px-8 py-6 text-left font-bold text-[#0050D8]/40 uppercase tracking-[0.15em] text-[10px]">Institutional Identity</th>
              <th class="px-6 py-6 text-left font-bold text-[#0050D8]/40 uppercase tracking-[0.15em] text-[10px]">Primary Contact</th>
              <th class="px-6 py-6 text-left font-bold text-[#0050D8]/40 uppercase tracking-[0.15em] text-[10px]">Location Node</th>
              <th class="px-6 py-6 text-left font-bold text-[#0050D8]/40 uppercase tracking-[0.15em] text-[10px]">Status</th>
              <th class="px-6 py-6 text-left font-bold text-[#0050D8]/40 uppercase tracking-[0.15em] text-[10px]">Launch Date</th>
              <th class="px-8 py-6 text-right font-bold text-[#0050D8]/40 uppercase tracking-[0.15em] text-[10px]">Management</th>
            </tr>
          </thead>
              ...
              <td class="px-8 py-6">
                <div class="flex items-center gap-4">
                  <div class="h-12 w-12 shrink-0 overflow-hidden rounded-[18px] border-2 border-white dark:border-neutral-800 bg-neutral-100 dark:bg-neutral-800 shadow-sm flex items-center justify-center transition-transform group-hover:scale-105">
                    ...
                    <div v-else class="text-[#0050D8] dark:text-white font-black text-sm">
                      {{ (group.name || 'SG').substring(0, 2).toUpperCase() }}
                    </div>
                  </div>
                  <div class="flex flex-col">
                    <span class="font-bold text-[#0050D8] dark:text-white text-base">{{ group.name || 'Unnamed Group' }}</span>
              ...
              <td class="px-8 py-6">
                <div class="flex items-center justify-end gap-3 opacity-0 group-hover:opacity-100 transition-all transform translate-x-2 group-hover:translate-x-0">
                  <button
                    ...
                    class="h-10 px-5 rounded-xl bg-white dark:bg-neutral-800 border border-neutral-100 dark:border-neutral-700 text-xs font-bold text-[#0050D8] dark:text-white hover:bg-neutral-50 transition-all shadow-sm"
                  >
                    Details
                  </button>
                  <button
