<template>
    <div class="h-[83vh]">
        <TableDrawer :showAddButton="false" :drawerWidth="(drawerTitle as any)?.width" :url="tableUrl"
            state="members-opening-balance" :drawerTitle="(drawerTitle as any)?.title"  :columns="columns">
            ...
            <template #id="{ item }">
                <div class=" sm:grid-cols-3 gap-2 px-4 py-2">
                    <label :key="item?.id"
...
const selected = ref<Record<string, any>>({}),
    drawerTitle = ref<any>({ title: 'Opening Balance Template', width: '600px' }),
...
const columns = [
    ...
    { key: 'account_number', label: 'account code', width: '14em', copy: true },
]
