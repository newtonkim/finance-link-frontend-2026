  ...
  try {
    ...
    rows.value = raw.slice(start)
      ...
      .map(r => ({
        ...
        opening_balance:String(r[2] ?? '').trim(),
        as_of_date:     String(r[3] instanceof Date ? `${r[3].getFullYear()}-${String(r[3].getMonth()+1).padStart(2,'0')}-${String(r[3].getDate()).padStart(2,'0')}` : (r[3] ?? '')).trim(),
  ...
  <div class="flex flex-col gap-5 p-6 max-w-5xl">
    ...
    <template v-if="step === 'upload'">
      <TemplateDownloadCard
        title="Step 1 — Download the Opening Balances template"
        description="Fill in member number, account number, opening balance, and the cut-over date."
        filename="opening-balances-template.xlsx"
        :download-fn="migrationApi.downloadOpeningBalancesTemplate"
      />
