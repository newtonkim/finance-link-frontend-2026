...
const allTabs: Array<{ key: LoanDetailTabKey; label: string; icon: Component }> = [
  { key: 'schedule', label: 'Payment Schedule', icon: markRaw(ClipboardList) },
  { key: 'reschedules', label: 'Reschedule History', icon: markRaw(History) },
  { key: 'transactions', label: 'Transaction History', icon: markRaw(History) },
  { key: 'general', label: 'General Information', icon: markRaw(CreditCard) },
  { key: 'before-topup', label: 'Before Topup', icon: markRaw(History) },
  { key: 'charges', label: 'Charges & Penalties', icon: markRaw(AlertTriangle) },
  { key: 'documents', label: 'Documents', icon: markRaw(FileText) },
  { key: 'activities', label: 'Loan Activities', icon: markRaw(Activity) },
]
...
export function useLoanScheduleData(
  ...
  const tabs = computed(() =>
    allTabs.filter((t) => {
      if (t.key === 'reschedules') return !!loan.value?.is_rescheduled
      if (t.key === 'before-topup') return !!loan.value?.parent_loan_id
      return true
    }),
  )
