...
import { loansApi, type LoanDetail, type LoanScheduleEntry, type LoanTransaction } from '@/tenant/apis/loans/loansApi'
...
export function useLoanAccount(loanId: number) {
    ...
    const activeTab = ref<'overview' | 'schedule' | 'repayments' | 'charges' | 'ledger' | 'documents'>('overview')
    ...
    async function fetchRepayments(page = 1) {
        const res = await loansApi.getRepayments(loanId, { page, per_page: repaymentsMeta.value.per_page })
    ...
    async function refresh() {
        try {
            await Promise.all([fetchLoan(), fetchSchedule(), fetchRepayments(repaymentsMeta.value.current_page)])
        } catch {
