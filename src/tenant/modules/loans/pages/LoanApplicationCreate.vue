...
const hasEligibilityFailures = computed(() =>
    !!(eligibilityResult.value && !eligibilityResult.value.eligible && eligibilityResult.value.failed.length > 0)
)
const canSubmit = computed(() => !hasEligibilityFailures.value)
...
<template>
    <div class="flex h-full flex-1 flex-col gap-6 bg-[#f8faf9] p-4 md:p-6 dark:bg-[#0a0a0a]">
        ...
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-neutral-900 dark:text-white">New Loan Application</h1>
                <p class="text-sm text-neutral-500 dark:text-neutral-400">Fill in the details and save as draft or submit for review.</p>
            </div>
        </div>
        ...
        <form @submit.prevent="save">
            ...
            <div v-show="currentStep === 1" class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
                ...
                <!-- Right: product info + eligibility -->
                <div class="flex flex-col gap-6">
                ...
                <div class="flex flex-col gap-6">
                    ...
                    <div class="rounded-2xl border border-neutral-100 bg-white p-6 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
                        ...
                        <div class="grid gap-4 sm:grid-cols-2">
                            <div>
                                ...
                                <p v-if="selectedProduct?.min_amount || selectedProduct?.max_amount" class="mt-1 text-xs text-neutral-400 dark:text-neutral-500">
                                    Range: {{ formatAmount(selectedProduct.min_amount) }} – {{ formatAmount(selectedProduct.max_amount) }}
                                </p>
                            </div>
                            <div>
                    ...
                    <div class="rounded-2xl border border-neutral-100 bg-white p-6 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
                        ...
                        <div v-if="!collateralItems.length"
                            ...
                            <p class="text-sm text-neutral-500 dark:text-neutral-400">No collateral added yet.</p>
                            <p class="mt-0.5 text-xs text-neutral-400 dark:text-neutral-500">Click "Add Item" to record collateral or securities for this loan.</p>
                        </div>
                                ...
                                <div class="grid gap-3 pr-6 sm:grid-cols-2">
                                    ...
                                    <div class="sm:col-span-2">
                                        ...
                                        <input v-model="item.description" type="text"
                                            placeholder="e.g. Title deed ref. LR1234, 2019 Toyota Corolla KAB 123X…"
                                            class="w-full rounded-xl border border-neutral-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-nfuko-primary/30 dark:border-neutral-700 dark:bg-neutral-800 dark:text-white" />
                                    </div>
                                    <div class="sm:col-span-2">
                                        <label class="mb-1 block text-xs font-medium text-neutral-600 dark:text-neutral-400">Notes</label>
                                        <input v-model="item.notes" type="text" placeholder="Additional notes…"
                                            class="w-full rounded-xl border border-neutral-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-nfuko-primary/30 dark:border-neutral-700 dark:bg-neutral-800 dark:text-white" />
                                    </div>
                                </div>
                            </div>
            ...
            <div v-show="currentStep === 3" class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
                ...
                <div class="flex flex-col gap-6">
                    ...
                    <div class="rounded-2xl border border-neutral-100 bg-white p-6 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
                        ...
                        <dl class="grid gap-2 text-sm">
                            <div class="flex justify-between gap-2 rounded-xl bg-neutral-50 px-4 py-3 dark:bg-neutral-800/50">
                                ...
                                <dd class="text-right font-medium text-neutral-900 dark:text-white">
                                    {{ selectedMember?.name ?? '—' }}
                                    <span v-if="selectedMember?.member_no" class="ml-1.5 text-xs text-neutral-400">({{ selectedMember.member_no }})</span>
                                </dd>
                            </div>
                            <div class="flex justify-between gap-2 rounded-xl bg-neutral-50 px-4 py-3 dark:bg-neutral-800/50">
                                <dt class="text-neutral-500 dark:text-neutral-400">Loan Product</dt>
                                <dd class="text-right font-medium text-neutral-900 dark:text-white">
                                    {{ selectedProduct?.name ?? '—' }}
                                    <span v-if="selectedProduct?.code" class="ml-1.5 text-xs text-neutral-400">{{ selectedProduct.code }}</span>
                                </dd>
                            </div>
                            <div class="flex justify-between gap-2 rounded-xl bg-neutral-50 px-4 py-3 dark:bg-neutral-800/50">
                                <dt class="text-neutral-500 dark:text-neutral-400">Requested Amount</dt>
                                <dd class="font-semibold text-nfuko-primary dark:text-bg-nfuko-yellow">{{ formatAmount(form.requested_amount) }}</dd>
                            </div>
                            <div class="flex justify-between gap-2 rounded-xl bg-neutral-50 px-4 py-3 dark:bg-neutral-800/50">
                                <dt class="text-neutral-500 dark:text-neutral-400">Requested Term</dt>
                                <dd class="font-medium text-neutral-900 dark:text-white">
                                    {{ form.requested_term ?? '—' }}
                                    <span v-if="selectedProduct?.duration_type" class="ml-1 text-xs text-neutral-400">{{ selectedProduct.duration_type }}</span>
                                </dd>
                            </div>
                            <div v-if="form.purpose" class="flex justify-between gap-2 rounded-xl bg-neutral-50 px-4 py-3 dark:bg-neutral-800/50">
                                <dt class="shrink-0 text-neutral-500 dark:text-neutral-400">Purpose</dt>
                                <dd class="max-w-xs text-right font-medium text-neutral-900 dark:text-white">{{ form.purpose }}</dd>
                            </div>
                            <div v-if="form.repayment_source" class="flex justify-between gap-2 rounded-xl bg-neutral-50 px-4 py-3 dark:bg-neutral-800/50">
                                <dt class="text-neutral-500 dark:text-neutral-400">Repayment Source</dt>
                                <dd class="text-right font-medium text-neutral-900 dark:text-white">{{ form.repayment_source }}</dd>
                            </div>
                        </dl>
                        <button type="button" class="mt-3 text-xs text-nfuko-primary hover:underline dark:text-bg-nfuko-yellow" @click="goToStep(1)">
                            Edit details
                        </button>
                    </div>

                    <!-- Collateral summary (if any) -->
                    <div v-if="collateralItems.length" class="rounded-2xl border border-neutral-100 bg-white p-6 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
                        <div class="mb-4 flex items-center justify-between">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="rounded-full bg-neutral-100 px-2 py-0.5 text-xs font-medium text-neutral-500 dark:bg-neutral-800 dark:text-neutral-400">{{ collateralItems.length }} item{{ collateralItems.length !== 1 ? 's' : '' }}</span>
                            </div>
                            <button type="button" class="text-xs text-nfuko-primary hover:underline dark:text-bg-nfuko-yellow" @click="goToStep(2)">Edit</button>
                        </div>
                        <div class="space-y-2">
                            <div v-for="(item, idx) in collateralItems" :key="idx"
                                class="flex items-start gap-3 rounded-xl bg-neutral-50 px-4 py-3 dark:bg-neutral-800/50">
                                <Shield class="mt-0.5 h-4 w-4 shrink-0 text-neutral-400 dark:text-neutral-500" />
                                <div class="min-w-0 flex-1">
                                    <p class="text-sm font-medium text-neutral-900 dark:text-white">{{ item.asset_type || 'Unknown type' }}</p>
                                    <p v-if="item.description" class="mt-0.5 truncate text-xs text-neutral-500 dark:text-neutral-400">{{ item.description }}</p>
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <button type="button" :disabled="submitting || saving || !canSubmit"
                                :title="!canSubmit ? 'Resolve eligibility failures before submitting' : undefined"
