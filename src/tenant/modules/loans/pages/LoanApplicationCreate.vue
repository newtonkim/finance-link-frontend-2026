...
const memberOptions  = computed(() => members.value.map(m => ({ ...m, name: `${m.name} (${m.member_no})` })))
const productOptions = computed(() => products.value.map(p => ({ ...p, name: `${p.name} (${p.code})` })))

function selectMember(m: any) { form.value.member_id = m.id; selectedMember.value = m }
function clearMember()        { form.value.member_id = null; selectedMember.value = null }
function selectProduct(p: any){ form.value.loan_product_id = p.id; void onProductChange() }
...
function onAmountBlur() {
    amountDisplay.value = form.value.requested_amount != null
        ? Number(form.value.requested_amount).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })
        : ''
}

// ─── Stepper ──────────────────────────────────────────────────────────────────
const currentStep = ref(1)
const steps = [{ id: 1, label: 'Member & Product' }, { id: 2, label: 'Loan Details' }, { id: 3, label: 'Review & Submit' }]
        ...
        <form @submit.prevent="save(collateralItems)">
            ...
            <div v-show="currentStep === 2" class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_480px]">
                <div class="flex flex-col gap-6">
                    <div class="rounded-2xl border border-neutral-100 bg-white p-6 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
                        ...
                        <div class="grid gap-4 sm:grid-cols-2">
                            <div>
                                ...
                                <p v-if="selectedProduct?.min_amount || selectedProduct?.max_amount" class="mt-1 text-xs text-neutral-400">Range: {{ formatAmount(selectedProduct.min_amount) }} – {{ formatAmount(selectedProduct.max_amount) }}</p>
                            </div>
                            <div>
                    ...
                    <LoanFormCollateralSection ref="collateralRef" v-model="collateralItems" :selected-product="selectedProduct" :selected-member="selectedMember" :requested-amount="Number(form.requested_amount) || null" />
