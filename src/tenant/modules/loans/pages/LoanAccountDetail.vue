...
import {
    ArrowLeft, Banknote, Calendar, CheckCircle2, AlertCircle,
    Loader2, ChevronLeft, ChevronRight, TrendingDown,
    ClipboardList, History, CreditCard, BookOpen, FileText,
...
const {
    loading, loan, schedule, repayments, repaymentsMeta,
    activeTab, refresh, fetchRepayments,
} = useLoanAccount(loanId)

const {
    showModal, posting, previewing,
    form, errors, preview,
    openModal, closeModal, loadPreview, submit, errMsg: previewErrMsg,
...
const tabs = [
    ...
    { key: 'repayments', label: 'Repayments', icon: History },
...
const hasNextRepayments = computed(() => repaymentsMeta.value.current_page < repaymentsMeta.value.last_page)
</script>
...
<template>
    <div class="px-4 py-6 sm:px-6 lg:px-8 space-y-5">
        ...
        <template v-else-if="loan">
            <!-- Loan header card -->
            <div class="rounded-2xl border border-neutral-100 bg-white dark:border-neutral-800 dark:bg-neutral-900 p-6">
                <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                    <div class="flex items-start gap-4">
                        <div class="flex h-12 w-12 items-center justify-center rounded-2xl bg-emerald-100 dark:bg-emerald-900/30">
                            <Banknote class="h-6 w-6 text-emerald-700 dark:text-emerald-400" />
                        </div>
                        <div>
                            <div class="flex items-center gap-2 flex-wrap">
                                <h1 class="text-lg font-bold font-mono text-neutral-900 dark:text-white">{{ loan.loan_no }}</h1>
                            ...
                            <p class="text-sm text-neutral-600 dark:text-neutral-400 mt-0.5">
                                {{ loan.member?.name }}
                                <span v-if="loan.member?.member_number" class="text-neutral-400"> · {{ loan.member.member_number }}</span>
                            </p>
                ...
                <div class="mt-5 grid grid-cols-2 sm:grid-cols-4 gap-4">
                    <div class="rounded-xl bg-neutral-50 dark:bg-neutral-800/40 p-4">
                        ...
                        <div class="font-bold text-neutral-900 dark:text-white">{{ fmt(loan.principal_formatted) }}</div>
                    </div>
                    <div class="rounded-xl bg-neutral-50 dark:bg-neutral-800/40 p-4">
                    ...
                    <div class="rounded-xl bg-neutral-50 dark:bg-neutral-800/40 p-4">
                        ...
                        <div class="font-bold text-neutral-900 dark:text-white">{{ fmt(loan.net_disbursed_amount_formatted) }}</div>
                    </div>
                    <div class="rounded-xl bg-neutral-50 dark:bg-neutral-800/40 p-4">
                        ...
                        <div class="mt-1.5 h-1.5 rounded-full bg-neutral-200 dark:bg-neutral-700">
                            <div class="h-1.5 rounded-full bg-emerald-500 transition-all" :style="{ width: repaidPercent + '%' }" />
                        </div>
                    </div>
                </div>
            </div>
            ...
            <!-- ── Overview ── -->
            <div v-if="activeTab === 'overview'" class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div class="rounded-2xl border border-neutral-100 bg-white dark:border-neutral-800 dark:bg-neutral-900 p-5">
                    <h3 class="text-xs font-semibold uppercase tracking-wide text-neutral-400 dark:text-neutral-500 mb-4">Loan Details</h3>
                    <dl class="space-y-3">
                        ...
                        <div class="flex justify-between text-sm">
                            ...
                            <dd class="text-neutral-900 dark:text-white">{{ fmt(loan.processing_fee_formatted) }}</dd>
                        </div>
                        <div class="flex justify-between text-sm">
                            <dt class="text-neutral-500 dark:text-neutral-400">Disbursement Method</dt>
                            <dd class="capitalize text-neutral-900 dark:text-white">{{ loan.disbursement_method?.replace(/_/g, ' ') ?? '—' }}</dd>
                        </div>
                ...
                <div class="rounded-2xl border border-neutral-100 bg-white dark:border-neutral-800 dark:bg-neutral-900 p-5">
                    <h3 class="text-xs font-semibold uppercase tracking-wide text-neutral-400 dark:text-neutral-500 mb-4">People</h3>
                    <dl class="space-y-3">
                        <div class="flex justify-between text-sm">
            ...
            <!-- ── Schedule ── -->
            <div v-if="activeTab === 'schedule'" class="rounded-2xl border border-neutral-100 bg-white dark:border-neutral-800 dark:bg-neutral-900 overflow-hidden">
                        ...
                        <tr v-for="row in schedule" :key="row.id"
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-1.5">
                                    ...
                                    <span class="text-neutral-700 dark:text-neutral-300">{{ fmtDate(row.due_date) }}</span>
                                </div>
                                <p v-if="row.is_overdue" class="text-xs text-red-500 mt-0.5">{{ row.days_overdue }}d overdue</p>
                            </td>
                            <td class="px-4 py-3 text-right text-neutral-700 dark:text-neutral-300">{{ fmt(row.principal_due_formatted) }}</td>
                            <td class="px-4 py-3 text-right text-neutral-700 dark:text-neutral-300">{{ fmt(row.interest_due_formatted) }}</td>
                            <td class="px-4 py-3 text-right font-medium text-neutral-900 dark:text-white">{{ fmt(row.total_due_formatted) }}</td>
                            <td class="px-4 py-3 text-right text-emerald-600 dark:text-emerald-400">
                                {{ fmt(Number(row.principal_paid) + Number(row.interest_paid) + Number(row.penalty_paid) + Number(row.charges_paid)) }}
                            </td>
                            <td class="px-4 py-3 text-right text-neutral-700 dark:text-neutral-300">{{ fmt(row.outstanding_balance_formatted) }}</td>
                            <td class="px-4 py-3">
            ...
            <div v-if="activeTab === 'repayments'" class="space-y-4">
                <div class="rounded-2xl border border-neutral-100 bg-white dark:border-neutral-800 dark:bg-neutral-900 overflow-hidden">
                    <div v-if="!repayments.length" class="flex flex-col items-center justify-center py-12 text-neutral-400 gap-2">
                    ...
                    <table v-else class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-neutral-100 dark:divide-neutral-800">
                            <tr v-for="txn in repayments" :key="txn.id"
                                class="hover:bg-neutral-50 dark:hover:bg-neutral-800/40">
                                <td class="px-4 py-3 font-mono text-xs text-neutral-500">
                                    {{ txn.receipt_no ?? txn.payment_id }}
                                </td>
                                <td class="px-4 py-3 text-neutral-700 dark:text-neutral-300">{{ fmtDate(txn.payment_date) }}</td>
                                <td class="px-4 py-3 text-right font-semibold text-emerald-600 dark:text-emerald-400">{{ fmt(txn.amount_paid_formatted) }}</td>
                                <td class="px-4 py-3 text-right text-neutral-600 dark:text-neutral-400">{{ fmt(txn.principal_portion_formatted) }}</td>
                                <td class="px-4 py-3 text-right text-neutral-600 dark:text-neutral-400">{{ fmt(txn.interest_portion_formatted) }}</td>
                                ...
                                <td class="px-4 py-3 text-neutral-600 dark:text-neutral-400">{{ txn.collected_by?.name ?? '—' }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                <!-- Repayments pagination -->
                <div v-if="repaymentsMeta.last_page > 1" class="flex items-center justify-between text-sm text-neutral-500">
                    <span>{{ repaymentsMeta.total }} repayments</span>
                    <div class="flex gap-2">
                        <button :disabled="!hasPrevRepayments"
                        ...
                        <button :disabled="!hasNextRepayments"
                            class="flex items-center gap-1 px-3 py-1.5 rounded-lg border border-neutral-200 hover:bg-neutral-50 disabled:opacity-40 disabled:cursor-not-allowed dark:border-neutral-700 dark:hover:bg-neutral-800"
                            @click="fetchRepayments(repaymentsMeta.current_page + 1)">
            ...
            <!-- ── Charges, Ledger, Documents — placeholder tabs ── -->
            <div v-if="activeTab === 'charges' || activeTab === 'ledger' || activeTab === 'documents'"
                ...
                <p class="text-sm capitalize">{{ activeTab }} — coming soon</p>
            </div>
        </template>
    </div>
