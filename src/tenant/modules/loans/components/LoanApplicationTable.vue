    ...
    <div class="rounded-2xl border border-neutral-100 bg-white shadow-sm dark:border-neutral-800 dark:bg-neutral-900 overflow-hidden">
        ...
        <div v-else class="divide-y divide-neutral-50 dark:divide-neutral-800 sm:hidden">
            <div v-for="app in applications" :key="app.id"
                ...
                <div class="flex items-start justify-between gap-2">
                    <div class="min-w-0">
                        <p class="font-mono text-xs font-semibold text-neutral-900 dark:text-white">{{ app.application_no ?? '—' }}</p>
                        <p class="mt-0.5 text-sm font-medium text-neutral-800 dark:text-neutral-200 truncate">{{ app.member?.name ?? '—' }}</p>
                        <p v-if="app.member?.member_no" class="text-xs text-neutral-400 dark:text-neutral-500">{{ app.member.member_no }}</p>
                    </div>
                    <span class="shrink-0 inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium capitalize" :class="statusBadgeClass(app.status)">
                ...
                <div class="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-neutral-500 dark:text-neutral-400">
                    ...
                    <span class="font-medium text-neutral-800 dark:text-neutral-200">{{ displayAmount(app.requested_amount_formatted, app.requested_amount) }}</span>
                    <span v-if="app.requested_term">{{ app.requested_term }}mo</span>
                    <span v-if="app.submitted_at">{{ formatDate(app.submitted_at) }}</span>
                    <span v-if="app.days_pending != null" class="inline-flex items-center rounded-full px-2 py-0.5 font-medium" :class="daysPendingClass(app.days_pending)">{{ app.days_pending }}d</span>
                </div>
                <div class="flex items-center gap-1 pt-1">
                    <button class="flex h-8 w-8 items-center justify-center rounded-lg hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors" @click="emit('view', app)">
                        <Eye class="h-4 w-4 text-neutral-500 dark:text-neutral-400" />
                    </button>
                    <template v-if="app.status === 'recommended'">
                        <button class="flex h-8 w-8 items-center justify-center rounded-lg text-green-600 hover:bg-green-50 dark:text-green-400 dark:hover:bg-green-900/20 transition-colors" @click="emit('approve', app)">
                            <ThumbsUp class="h-4 w-4" />
                        </button>
                        <button class="flex h-8 w-8 items-center justify-center rounded-lg text-red-600 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-900/20 transition-colors" @click="emit('decline', app)">
                            <ThumbsDown class="h-4 w-4" />
                        </button>
                    </template>
                    <button v-if="app.status === 'draft'" class="flex h-8 w-8 items-center justify-center rounded-lg hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors" @click="emit('edit', app)">
                        <Pencil class="h-4 w-4 text-neutral-500 dark:text-neutral-400" />
                    </button>
                    <button v-if="app.status === 'cancelled'" class="flex h-8 w-8 items-center justify-center rounded-lg text-emerald-600 hover:bg-emerald-50 dark:text-emerald-400 dark:hover:bg-emerald-900/20 transition-colors" @click="emit('reopen', app.id!)">
        ...
        <table v-if="!loading && applications.length > 0" class="hidden w-full text-sm sm:table">
            ...
            <tbody class="divide-y divide-neutral-50 dark:divide-neutral-800">
                <tr v-for="app in applications" :key="app.id" class="hover:bg-neutral-50 dark:hover:bg-neutral-800/50 transition-colors">
                    <td class="px-6 py-4 font-mono text-xs font-medium text-neutral-900 dark:text-white">{{ app.application_no ?? '—' }}</td>
                    <td class="px-6 py-4">
                        <div class="font-medium text-neutral-900 dark:text-white">{{ app.member?.name ?? '—' }}</div>
                        <div v-if="app.member?.member_no" class="mt-0.5 text-xs text-neutral-400 dark:text-neutral-500">{{ app.member.member_no }}</div>
                    </td>
                    <td class="px-6 py-4 text-neutral-600 dark:text-neutral-400">{{ app.loan_product?.name ?? '—' }}</td>
                    ...
                    <template v-if="isRecommendedView">
                        <td class="px-6 py-4 text-right">
                            <span class="font-medium text-purple-700 dark:text-purple-300">{{ displayAmount(app.recommended_amount_formatted, app.recommended_amount) }}</span>
                            <div v-if="app.recommended_term" class="text-xs text-neutral-400">{{ app.recommended_term }}mo</div>
                        </td>
                        <td class="px-6 py-4 text-right text-neutral-500 dark:text-neutral-400">{{ displayAmount(app.loan_product?.max_amount_formatted, app.loan_product?.max_amount) }}</td>
                        <td class="px-6 py-4 text-center">
                            <span v-if="app.risk_rating" class="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium capitalize"
                                :class="{
                        ...
                        <td class="px-6 py-4 text-center text-neutral-500 dark:text-neutral-400 text-xs">{{ app.approvals_count ?? 0 }} cast</td>
                    </template>
                    <template v-else>
                        <td class="px-6 py-4 text-right text-neutral-600 dark:text-neutral-400">{{ displayAmount(app.approved_amount_formatted, app.approved_amount) }}</td>
                        <td class="px-6 py-4">
                            <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium capitalize" :class="statusBadgeClass(app.status)">
                                {{ statusLabel(app.status) }}
                            </span>
                        </td>
                    </template>
                    ...
                    <td class="px-6 py-4 text-center">
                        <span v-if="app.days_pending != null" class="inline-flex min-w-[2rem] items-center justify-center rounded-full px-2 py-0.5 text-xs font-medium" :class="daysPendingClass(app.days_pending)">
                            {{ app.days_pending }}
                        </span>
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center justify-end gap-1">
                            <button class="flex h-8 w-8 items-center justify-center rounded-lg hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors" title="View" @click="emit('view', app)">
                                <Eye class="h-4 w-4 text-neutral-500 dark:text-neutral-400" />
                            </button>
                            <template v-if="app.status === 'recommended'">
                                <button class="flex h-8 w-8 items-center justify-center rounded-lg text-green-600 hover:bg-green-50 dark:text-green-400 dark:hover:bg-green-900/20 transition-colors" title="Approve" @click="emit('approve', app)">
                                    <ThumbsUp class="h-4 w-4" />
                                </button>
                                <button class="flex h-8 w-8 items-center justify-center rounded-lg text-red-600 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-900/20 transition-colors" title="Decline" @click="emit('decline', app)">
                                    <ThumbsDown class="h-4 w-4" />
                                </button>
                            </template>
                            <button v-if="app.status === 'draft'" class="flex h-8 w-8 items-center justify-center rounded-lg hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors" title="Edit" @click="emit('edit', app)">
                                <Pencil class="h-4 w-4 text-neutral-500 dark:text-neutral-400" />
                            </button>
                            <button v-if="app.status === 'cancelled'" class="flex h-8 w-8 items-center justify-center rounded-lg text-emerald-600 hover:bg-emerald-50 dark:text-emerald-400 dark:hover:bg-emerald-900/20 transition-colors" title="Reopen as Draft" @click="emit('reopen', app.id!)">
                                <RotateCcw class="h-4 w-4" />
                            </button>
                        </div>
                    </td>
    ...
    <div v-if="meta.total > 0" class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div v-if="meta.last_page > 1" class="flex items-center gap-1">
            ...
            <template v-for="(p, i) in visiblePages" :key="i">
                <span v-if="p === null" class="flex h-8 w-6 items-end justify-center pb-1 text-sm text-neutral-400 dark:text-neutral-600">…</span>
                <button v-else
