...
async function viewDocument(doc: UploadedDoc) {
  ...
  try {
    ...
    toast.error('Failed to open document. Ensure the backend has a download endpoint.')
  } finally {
    viewing.value = null
  }
}
    ...
    <template v-else>
      ...
      <div v-if="required.length" class="space-y-3">
        <div v-for="req in required" :key="req.slug">
          ...
          <p
            v-else
            class="rounded-xl border border-dashed border-neutral-200 px-3 py-2 text-xs text-neutral-400 dark:border-neutral-700 dark:text-neutral-500"
          >
            Not yet uploaded
          </p>
        </div>
