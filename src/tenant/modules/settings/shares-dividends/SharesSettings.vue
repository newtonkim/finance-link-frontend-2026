...
const settingsCards = [
  ...
  },
  {
    ...
    title: "Share Transaction Charges",
...
<template>
  <div
    class="flex h-full flex-1 flex-col gap-6 bg-[#f8faf9] p-4 dark:bg-[#0a0a0a] md:p-6"
  >
    <!-- Page Header -->
    ...
    <SettingCard :settingsCards="settingsCards">
      ...
      <template #dividend-distribution>
        <div class="mb-1 flex flex gap-3">
          <div
          ...
          >
            <Users class="h-3 w-3" />
            <span>{{
              dividendForm.distribution_account_type === "shareholders_only"
                ? "Share holders only"
                : "All accounts"
            }}</span>
          </div>
          ...
          <div
            v-if="dividendForm.dividend_rate"
            class="flex items-center gap-1.5 rounded-lg border border-neutral-100 bg-neutral-50 px-3 py-1.5 text-[12px] text-neutral-500 dark:border-neutral-700 dark:bg-neutral-800"
          >
