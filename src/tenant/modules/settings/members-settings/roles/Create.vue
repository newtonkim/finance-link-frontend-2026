     ...
     if (props.data?.id) {
        ...
        for (let i = 0; i < colection.length; i++) {
            ...
            if (element.name !== 'permission') 
                form.value[i].value = (props.data[element.name]);
...
<template>
    <div class="">
        <Card
            ...
            <Form :action="data.action" v-if="!loading" v-model:form="form"
                parentStyle="grid grid-cols-2 sm:grid-cols-1 gap-4 md:gap-6 px-4 py-0" />
            <div class="h-[40vh] overflow-auto">
                <Table :action_config="ACTION_CONFIG" :handleAction="handleAction" :dataFilter="permissionSelected" :columns="columns">
                </Table>
            </div>
        </Card>
    </div>
