<template>
  <div v-for="item in links" :key="item.path">
    ...
    <div
        ...
        max-w-[50em]  flex absolute left-full top-0 mt-0 ml-6   rounded-xl py-2 shadow-md shadow-black/20 border  transition-all z-[9999]   dark:bg-[#001d22]"
      >
      <template
          v-for="(child, index) in item.children"
          :key="index">
         <div  v-auth="subItem?.permissions"
          class="w-[40em] items-center gap-4 px-6 py-1"
        >
     
          <div
            class="w-full flex items-center my-1 gap-0 px-4 py-2 rounded-md bg-[#001d22]/90  dark:text-white"
          >
            {{ child.title }}
          </div>
          <RouteLink
