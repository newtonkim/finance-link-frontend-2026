        ...
        <thead v-once class="sticky top-0 z-40 bg-white  shadow-sm hover dark:bg-neutral-900  rounded-sm ">
            <tr class="px-4 py-3 font-bold text-neutral-700 dark:text-white ">
                <th v-for="col in columns" :key="col.key" :class="[
                    'py-3 px-3  text-sm font-semibold text-gray-700 dark:text-white u  border-b border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800  text-[14px]  capitalize  dark:text-white font-semibold border-b dark:border-slate-700 bg-neutral-50/50 dark:bg-neutral-600/50 border-b border-neutral-100 dark:border-neutral-800',
        ...
        <tbody class="">
            ...
            <tr v-for="(item, idx) in dataFilter" :key="item?.id ?? idx"
                ...
                <td v-for="col in columns" :key="col.key" :class="[
                    ...
                    col.sticky ? `sticky z-30 ${col.sticky}-0 dark:bg-neutral-900 bg-white  dark:bg-neutral-900` : '',
                    col.width ? `w-[${col.width}px]` : '',
                    col.class || '',
                    col.key === 'actions' ? 'text-center sticky z-30 right-0 dark:bg-neutral-900 bg-white  dark:bg-neutral-900' : '',
                ]
