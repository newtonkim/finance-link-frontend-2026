<template>
    <div class="flex-grow ">
        ...
        <table class="w-full w-full text-left border-collapse p-5 table-fixed">
            <!-- HEAD -->
                ...
                <tr v-for="(item, idx) in dataFilter" :key="item?.id ?? idx"
                    ...
                    <td v-for="col in columns" :key="col.key" :class="[
                        ...
                        <template v-else>
                            ...
                            <span v-else
                                v-html="col.type ? dataFomater(item[col.key], col.type) : item[col.key]"></span>
                        </template>
                    </td>
...
function getColumnStyle(col: any) {
    const width =
        typeof col.width
            ? col.width + 'em' // convert numeric to percentage
            : col.width;


    return {
        width,
        minWidth: col.width ? (typeof col.width === 'number' ? col.width + 'em' : col.width) : undefined,
        ...(col.style || {})
    };
}
</script>
