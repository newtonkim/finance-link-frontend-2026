<template>
  <div
    class="w-full dark:bg-neutral-900 bg- nfuko-50 overflow-x-auto w-full custom-scrollbar h-[64vh] border-neutral-100 bg-white dark:bg-neutral-900 shadow-sm dark:border-neutral-800 rounded-2xl">
    <table class="w-full table-auto text-left border-collapse">
      <thead v-once class="sticky top-0 z-40 bg-white shadow-sm dark:bg-neutral-900">
      ...
      <tbody class="over flow-auto table-auto text-left border-collapse min-w-max">
        <tr v-if="!dataFilter?.length">
          <td :colspan="columns.length + ($slots.actions ? 1 : 0)" class="py-8 text-center text-sm text-neutral-500">
            <EmptySvg />
          </td>
        </tr>
