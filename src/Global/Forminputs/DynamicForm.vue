...
onMounted(() => {
  if (Array.isArray(props.form)) prfields.value = [...props.form];
...
function DatawhistleBlower(newFields) {
  ...
  newFields.forEach((field: any) => {
    ...
    } else NewCollectionSet.push(field);
  });
...
function shouldShowField(field: any) {
  ...
  if (field.dependsOn.conditions) {
    ...
    const results = conditions.map((condition: any) => {
      const target = prfields.value.find((f: any) => f.name === condition.field);
              ...
              <span v-else>
                  ...
                  <div class="flex">
                    <input
                      ...
                      :class="[
                        ...
                        field?.class,
                        field.suffix
                          ? 'flex-1 rounded-xl rounded-r-none border-neutral-200 dark:border-white/10 dark:bg-[#0a0a0a] dark:text-white'
                          : '',
                      ]"
                ...
                <template v-else-if="field.type === 'money'">
                  <div class="flex">
                    ...
                    <div
                      v-if="field.error"
                      class="mt-2 px-1 text-xs text-red-500 font-medium"
                    >
                      {{ field.error }}
                    </div>
                  </div>
                ...
                <!-- Avatar -->
                <template
                  v-else-if="['avatar', 'avatar2', 'profile'].includes(field.type)"
                >
                  <div class="flex items-center gap-4 mt-2" :class="[field?.class]">
                    ...
                    >
                      <img
                        v-if="avatarPreviews[index] || field.value"
                        :src="avatarPreviews[index] || field.value"
                        alt="Avatar"
