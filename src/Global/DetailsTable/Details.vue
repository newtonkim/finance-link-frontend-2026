<template>
    <div class="space-y-4  capitalize ">
        ...
        <section v-for="(section, sIndex) in columns" :key="sIndex" class="overflow-hidden rounded-lg r border-neutral-200 dark:border-neutral-700   dark:bg-neutral-900  
            ...
            <Table v-if="section.type.toLocaleLowerCase() === 'table'" :handleAction="handleAction" :action_config="ACTION_CONFIG" :dataFilter="section?.list" :data="section?.list" :columns="section.column" />
            <div v-else-if="section.type.toLocaleLowerCase() === 'descriptions'" class="grid gap-[1px] bg-neutral-200 dark:bg-neutral-700"
                :style="gridStyle(section.column)">
                <div v-for="item in section.list" :key="item.key" class="flex flex-col bg-white  dark:bg-neutral-900 px-4 py-2.5 transition hover:bg-neutral-50 dark:hover:bg-neutral-800">
                    <span class="text-[10px] font-semibold tracking-wide text-neutral-400 uppercase mb-0">
                        {{ t(item.label) }}
                    </span>
                    <div class="text-sm font-medium text-neutral-700 dark:text-neutral-200 break-words" :class="{'text-right': item.align === 'right','text-center': item.align === 'center'}">
