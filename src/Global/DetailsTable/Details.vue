        ...
        <section v-for="(section, sIndex) in columns" :key="sIndex" class="overflow-hidden rounded-lg border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 shadow-sm transition-all
            ...
            <div v-if="section.type === 'Descriptions'" class="grid gap-[1px] bg-neutral-200 dark:bg-neutral-700"
                ...
                <div v-for="item in section.list" :key="item.key"
                    class="flex flex-col bg-white dark:bg-neutral-900 px-4 py-2 transition hover:bg-neutral-50 dark:hover:bg-neutral-800">

                    <span class="text-[10px] font-semibold tracking-wide text-neutral-400 uppercase mb-0">
                        {{ t(item.label) }}
                    </span>

                    <div class="text-sm font-medium text-neutral-700 dark:text-neutral-200 break-words" :class="{
                        ...
                        <template v-else>
                            ...
                            <template v-else>
                                <span
                                    v-html="item.type ? dataFomater(data[item.key], item.type) : renderValue(item)"></span>
                            </template>
