                            ...
                            <div
                                ...
                                <div v-else class="relative z-10 overflow-hidden border p-5">
                                    <div v-html="qrCodeSvg"
                                        class="flex aspect-square size-full items-center justify-center" :style="{
                                            filter:
...
const confirmTwoFactor = async () => {
    ...
    try {
        await apiClient.post('/user/confirm-two-factor-authentication', {
            code: code.value,
        });
        isOpen.value = false;
        // The parent component should handle the 'enabled' state update
    } catch (error: any) {
                    ...
                    <form @submit.prevent="confirmTwoFactor" class="w-full space-y-3">
                        <div ref="pinInputContainerRef" class="relative w-full space-y-3">
                            <div class="flex w-full flex-col items-center justify-center space-y-3 py-2">
                                ...
                                <InputError v-if="Object.keys(errors).length" :message="errors[0]" />
                            </div>
