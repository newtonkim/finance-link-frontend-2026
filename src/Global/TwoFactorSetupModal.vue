...
import { InputOTP, InputOTPGroup, InputOTPSlot } from '@/Global/ui/input-otp';
...
const confirmTwoFactor = async () => {
  ...
  try {
    // Placeholder flow: API integration can be wired in useTwoFactorAuth.
    isOpen.value = false;
  ...
  <Dialog :open="isOpen" @update:open="isOpen = $event">
    <DialogContent class="sm:max-w-md">
      ...
      <div class="relative flex w-auto flex-col items-center justify-center space-y-5">
        ...
        <template v-else>
          <form @submit.prevent="confirmTwoFactor" class="w-full space-y-3">
            <div ref="pinInputContainerRef" class="relative w-full space-y-3">
              <div class="flex w-full flex-col items-center justify-center space-y-3 py-2">
                ...
                <InputError v-if="errors.length" :message="errors[0]" />
              </div>

              <div class="flex w-full items-center space-x-5">
                <Button type="button" variant="outline" class="w-auto flex-1" @click="showVerificationStep = false" :disabled="processing">
                  Back
                </Button>
                <Button type="submit" class="w-auto flex-1" :disabled="processing || code.length < 6">
                  <template v-if="processing">Confirming...</template>
