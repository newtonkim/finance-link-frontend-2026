  ...
  <DialogRoot :open="show" @update:open="(v) => emit('update:show', v)">
    ...
    <DialogPortal>
      ...
      <DialogOverlay class="fixed inset-0 z-[9999] bg-black/50 backdrop-blur-sm
               ...
               data-[state=open]:fade-in-0" @click="cancelDelete" />

      <!-- Dialog Content -->
      <DialogContent class="fixed left-1/2 top-1/2 z-[10000] w-full max-w-md
        ...
        <div class="flex items-start gap-4">
          <div :class="currentStyle?.color" class="flex h-10 w-10 items-center justify-center rounded-full">
            <component :is="currentStyle?.icon" class="h-4 w-4" />
          </div>

          <div>
            <h3 class="text-[14px] capitalize  font-semibold text-neutral-900 dark:text-white mb-5">
            ...
            <div v-if="!$slots.message && !$slots.body"
              class="text-[12px] capitalize text-neutral-500 dark:text-neutral-200 ml-3 capitalize">
        ...
        <div class="mt-6 flex justify-end gap-3">
          <DialogClose as-child>
            <button class="rounded-lg border border-neutral-200 px-4 py-2 text-sm
          ...
          <button :class="[currentStyle?.bg]" @click="executeDelete(items)" class="flex items-center gap-2 rounded-lg px-4 py-2 text-sm
                   text-white disabled:opacity-50">
            <span v-if="deleting" class="h-4 w-4 animate-spin rounded-full border-2 border-white/20 border-t-white" />
