# Reschedule Loan Drawer Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the centred `LoanRescheduleModal` (Dialog) with a right-side drawer that never scrolls and lays all fields out compactly within a single viewport height.

**Architecture:** A new `LoanRescheduleDrawer.vue` is extracted as a self-contained component following the exact `LoanDisbursementDrawer.vue` pattern (Teleport + Transition, `fixed inset-y-0 right-0 flex flex-col`). `LoanAccountDetail.vue` replaces the old modal `ref.show()` pattern with a `v-model:open` boolean. All existing three-step logic (Configure → Preview → Success) is preserved; the only changes are layout, structure, and container.

**Tech Stack:** Vue 3 `<script setup>` + TypeScript, Tailwind CSS v4, Lucide Vue Next, existing `loansApi`.
...
|---|---|---|
| Create | `src/tenant/modules/loans/components/LoanRescheduleDrawer.vue` | Full drawer — 3-step reschedule flow, no scroll |
| Modify | `src/tenant/modules/loans/pages/LoanAccountDetail.vue` | Swap modal import/ref for drawer + v-model:open |

---

## Task 1: Create LoanRescheduleDrawer.vue

**Files:**
- Create: `src/tenant/modules/loans/components/LoanRescheduleDrawer.vue`

- [ ] **Step 1: Create the file with the complete implementation**

Create `/Users/asd/Mfuko_pro/mfuko-pro-frontend-2026/src/tenant/modules/loans/components/LoanRescheduleDrawer.vue` with the following content:
...
```bash
test -f src/tenant/modules/loans/components/LoanRescheduleDrawer.vue && echo "✓ file exists"
```
...
git add src/tenant/modules/loans/components/LoanRescheduleDrawer.vue
git commit -m "feat(loans): add LoanRescheduleDrawer replacing scrollable modal"
```

---

## Task 2: Wire LoanRescheduleDrawer into LoanAccountDetail.vue

**Files:**
- Modify: `src/tenant/modules/loans/pages/LoanAccountDetail.vue`

The changes are surgical — 4 targeted edits:
1. Replace import of `LoanRescheduleModal` with `LoanRescheduleDrawer`
2. Replace `rescheduleModalRef = ref<any>(null)` with `rescheduleDrawerOpen = ref(false)`
3. Replace `handleReschedule()` body (it now just sets the boolean)
4. Replace the `<LoanRescheduleModal ref="rescheduleModalRef" ...>` usage at the bottom of the template

- [ ] **Step 1: Replace the import (line 47)**
...
- [ ] **Step 2: Replace the ref and handler (around lines 506-519)**
...
- [ ] **Step 3: Replace the template usage (last lines of template, around line 2177)**
...
```bash
git add src/tenant/modules/loans/pages/LoanAccountDetail.vue
git commit -m "refactor(loans): swap LoanRescheduleModal for LoanRescheduleDrawer with v-model:open"
```
...
| Requirement | Task |
|---|---|
| Drawer (not modal) | Task 1 — Teleport + Transition slide-in from right ✅ |
| Not scrollable (drawer body) | Task 1 — no `overflow-y-auto` on the outer body div; only the schedule table has internal scroll ✅ |
| All date and input fields fit | Task 1 — compact 3-col grid for waivers+date; type input takes one row; loan summary bar 1 row; type cards 1 row; reason textarea fills remaining space ✅ |
| `LoanAccountDetail.vue` wired up | Task 2 ✅ |

**Placeholder scan:** No TBDs, no "add validation" phrases, no incomplete sections.

**Type consistency:** `emit('update:open', false)` matches `v-model:open` binding in Task 2. `@success="refresh"` is unchanged from the old modal. `loan` prop type stays `any` consistent with the original.
